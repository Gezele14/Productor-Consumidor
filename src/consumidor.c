...
{
  system("clear");
  printc("Consumidor | Productor-Consumidor\n\n", 6);
  ...
  while (1)
  {
    ...
    Variables[0].readIndex = usedMemSpace(buffer, Variables[0].memSize, Variables[0].readIndex);

    char text1[200];
    sprintf(text1, "<M> Leido del bloque %d:\n", Variables[0].readIndex);
    printc(text1, 6);

    char text2[200];
    sprintf(text2, "\t- %s|%s ", buffer[Variables[0].readIndex].date, buffer[Variables[0].readIndex].hour);
    printc(text2, 6);

    char text3[200];
    sprintf(text3, " - %s\n\t- Productores activos: %d\n\t- Consumidores activos: %d\n\n", buffer[Variables[0].readIndex].msg, Variables[0].producers, Variables[0].consumers);
    printc(text3, 6);

    buffer[Variables[0].readIndex].used = 0;
    ...
    if (buffer[Variables[0].readIndex].magic_number == getpid() % 6)
    {
      gettimeofday(&end, NULL);
      double tiempo = (end.tv_sec - start.tv_sec);
